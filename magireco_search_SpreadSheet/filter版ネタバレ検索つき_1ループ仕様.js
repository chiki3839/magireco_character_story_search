...
/*
  var and_or = "AND";
  var storyAry = "メイン【第2部】".split(',');
  var charaAry = "時女静香".split(',');
  var star     = "false";
  var keywordtext = "";
*/
   ...
   */
    ...
    storyAry.forEach(storyName =>{
      ...
      outData[storyName] = dataObj[storyName].filter(storyLine =>{
        return (!keywordtext && mgirlflg( storyLine[1], charaAry, and_or, addGirl)
        || !charaAry[0] && wordmuchflg(wordtextAry, storyLine[2])
        || mgirlflg( storyLine[1], charaAry, and_or, addGirl) && wordmuchflg(wordtextAry, storyLine[2])); 
      });
    });
